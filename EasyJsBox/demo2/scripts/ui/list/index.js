...
class ListUI extends ListData {
    ...
    setDelegate() {
        this.delegates.setEditingCallback(status => {
            const editButton = this.navigationView.navigationBarItems.getButtons().find(button => button.id === this.editButtonId)
            if (editButton) {
    ...
    updateList(reload = false) {
        ...
        listView.data = this.displayItems.map(item =>
            this.views.lineData(item, this.copied.uuid === item.uuid)
        )
