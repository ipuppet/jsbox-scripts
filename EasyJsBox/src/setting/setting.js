...
class Setting extends Controller {
  ...
  loader(item) {
    ...
    switch (item.type) {
      ...
      case "string":
        ...
        break
      case "stepper":
        settingItem = new SettingStepper(item).with({
          min: item.min ?? 1,
          max: item.max ?? 12
        })
        break
      case "script":
        ...
        settingItem = new SettingScript(item).with({
          script: item.script ?? item.value
        })
        break
      case "tab":
        settingItem = new SettingTab(item).with({
          items: item.items,
          values: item.values
        })
        break
      ...
      case "icon":
        ...
        break
      case "push":
        settingItem = new SettingPush(item).with({
          view: item.view,
          navButtons: item.navButtons
        })
        break
      case "child":
  ...
  getOriginal(key, _default = null) {
    ...
    if (Object.prototype.hasOwnProperty.call(this.setting, key)) {
      if (typeof this.setting[key] === "function") {
  ...
  getListView(structure = this.structure, footer = this.footer, id = this.name) {
    return {
      ...
      props: {
        id,
        style: 2,
        separatorInset: $insets(0, SettingItem.iconSize + SettingItem.edgeOffset * 2, 0, SettingItem.edgeOffset), // 分割线边距
        footer: footer,
